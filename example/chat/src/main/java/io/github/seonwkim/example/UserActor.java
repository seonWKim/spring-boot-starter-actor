...
/**
 * Actor that represents a user in the chat system.
 * It receives events from chat rooms and forwards them to the user's WebSocket session.
 */
@Component
public class UserActor implements SpringActor {
    ...
    /**
     * Creates a behavior for a user actor.
     * This method is called by the actor system when a new user actor is created.
     *
    ...
    private static class UserActorBehavior {
        ...
        UserActorBehavior(ActorContext<ChatRoomActor.ChatEvent> context, WebSocketSession session, ObjectMapper objectMapper) {
        ...
        private Behavior<ChatRoomActor.ChatEvent> onMessageReceived(ChatRoomActor.MessageReceived event) {
