...
public class ActorClusterMetricsExporter {
    ...
    private final Set<Class<?>> targetClasses =
            Set.of(ChatRoomActor.JoinRoom.class, ChatRoomActor.LeaveRoom.class, ChatRoomActor.SendMessage.class);
    ...
    public void registerMetrics() {
        InvokeAdviceEventInterceptorsHolder.register(new InvokeAdviceEventInterceptor() {
            ...
            public void onExit(Object envelope, long startTime) {
                ...
                Timer timer =
                        invokeTimers.computeIfAbsent(messageType, mt -> Timer.builder("pekko.actorcell.invoke.timer")
                                .description("Time spent in ActorCell.invoke(Envelope)")
                                .tags("messageType", mt)
                                .register(registry));

                Counter counter = invokeCounters.computeIfAbsent(
                        messageType, mt -> Counter.builder("pekko.actorcell.invoke.count")
