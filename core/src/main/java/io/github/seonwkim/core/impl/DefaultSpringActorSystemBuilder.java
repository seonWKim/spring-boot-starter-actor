...
public class DefaultSpringActorSystemBuilder implements SpringActorSystemBuilder {
	 ...
	 */
	@Override
	public SpringActorSystemBuilder withShardedActorRegistry(
	...
	public SpringActorSystem build() {
		final Config config =
				ConfigFactory.parseMap(ConfigValueFactory.fromMap(applyDefaultSerializers(configMap)))
						.withFallback(ConfigFactory.load());
		final String name =
				config.hasPath("pekko.name") ? config.getString("pekko.name") : DEFAULT_SYSTEM_NAME;

		final ActorSystem<RootGuardian.Command> actorSystem =
				ActorSystem.create(supplier.getSupplier().get(), name, config);
		final boolean isClusterMode =
				Objects.equals(config.getString("pekko.actor.provider"), "cluster");
		...
		if (!isClusterMode) {
			...
			return new SpringActorSystem(
					actorSystem, cluster, clusterSharding, applicationEventPublisher);
		}
	}
	...
	private Map<String, Object> applyDefaultSerializers(Map<String, Object> configMap) {
		...
		defaultBindings.put(
				"io.github.seonwkim.core.serialization.JsonSerializable", jacksonJsonSerializerName);
		defaultBindings.put(
				"io.github.seonwkim.core.serialization.CborSerializable", jacksonCborSerializerName);

		// Get or create pekko.actor.serializers
		Map<String, Object> pekko =
				(Map<String, Object>) result.computeIfAbsent("pekko", k -> new HashMap<>());
		Map<String, Object> actor =
				(Map<String, Object>) pekko.computeIfAbsent("actor", k -> new HashMap<>());
